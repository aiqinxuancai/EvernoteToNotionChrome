...
namespace HTMLToNotion
{
    ...
    public partial class MainWindow : FluentWindow
    {
        ...
        private async void StartDocXWithPath(string path)
        {
        ...
        private async void StartHTMLWithPath(string path)
        {
            var docxPath = Path.Combine(path, "HTML");
            ...
            if (Directory.Exists(path))
            {
                ...
                await this.Dispatcher.Invoke(async () =>
                {
                    ...
                    var result = await service.ShowSimpleDialogAsync(
                        ...
                        {
                            Title = "成功",
                            Content = "完成，处理完成的HTML存储在\\HTML\r\n只需将.html文件导入Notion既可",
                            PrimaryButtonText = "打开目录",
                            CloseButtonText = "关闭",
                        }
                    );
